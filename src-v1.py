...
# ## We allow user to choose one of three models, with default model being RF Regressor. We will define our pipeline for deployment using make_pipeline in six stages:
# 
...
models = {}
    if 'Model_Choice' in config:
        model_choices = config['Model_Choice']
        for key in model_choices:
            models[key] = model_choices[key]

# Display the Model_choice to the user
print("Model_choices are :")
for key, value in models.items():
    print(f"{key}: {value}")

user_model = config["Model_Choice"]["user_model"]

print("In the config.ini file, you have chosen :", user_model, "dtype: ", type(config["Model_Choice"]["user_model"]))

# def get_model_from_user():
    """
    Function to ask the user to input a model choice

    Returns:
    user_model (str)
    """
    # model_choice = user_model
    # model_choice = input("The Default Model is RandomForest, if you wish to change this, please input your model_choice using the keys, otherwise type 'rf': ")
    # return model_choice

# Example usage:
# if __name__ == "__main__":
    # user_model = get_model_from_user()
    # print("Your model choice is:", user_model)

# In[31]:

# Ask User for URL to training datafile and fetch data from URL. Alternatively, read train_url from config-ini file
...
if __name__ == "__main__":
    ...
    print("Training Data url is:", user_url)
...
# In[40]:
# user_model = RandomForestRegressor(max_depth=10)  ## this works but not config[Model_Choice][user_model]
# because config returns a string and not an object
# from <class 'sklearn.ensemble._forest.RandomForestRegressor'>
# How to fix?

# Train different algorithms based on user_model
if user_model == 'rf':
...
# In[41]:
print("user_model is: ", user_model, type(user_model))
...
if __name__ == "__main__":
    user_url = get_url_from_user()
    print("Prediction data url is: ", user_url)
...
if __name__ == "__main__":
    ...
    print("Predicted final_test scores are saved to file path:", file_path)
...
print("Final_test scores have been properly saved to: ", file_path)
